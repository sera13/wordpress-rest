...
public class RetreiveWordpressInfoService {
    ...
    public List<Post> getAllPosts(String kindOfPost) {

        RestTemplate restTemplate = new RestTemplateBuilder().basicAuthentication("serafeim", "NFBN57Z8sVXs!a1N(IsFMdT(").build();
        List<Post> posts = new ArrayList<>();
        int index = 1;
        while (true) {
            Post[] partialPosts = restTemplate.getForObject("http://localhost:8081/wp-json/wp/v2/" + kindOfPost + "?per_page=100&page=" + index, Post[].class);
